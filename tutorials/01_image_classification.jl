...
# ╔═╡ 215610fe-3f5b-11ef-36e4-111f43e94806
begin
	using Printf
	using Random
...
# ╔═╡ 5adde1b8-28ed-466c-a6bf-f092d3ccef03
using PlutoUI

# ╔═╡ b8c06b78-03bf-4cd5-9d57-8a222c4a58af
md"""
# Image Classification
"""

# ╔═╡ 1ba3fd93-b587-433e-a70f-292274959414
md"""
Image classification is a typical task in computer vision and MNIST is used for training and testing handwritten digit recognition models. In this notebook, we will build a handwritten digit recognition model using [Lux.jl](https://lux.csail.mit.edu/stable/), a pure Julia deep learning framework.
"""

# ╔═╡ a493bc43-7455-4c4a-b721-affd6fd89b04
md"""
## Install packages
"""

# ╔═╡ c5e9d330-5a3d-4603-ac99-9e635ee96a7c
md"""
We download MNIST dataset using MLDatasets.
"""

# ╔═╡ 94136bec-5c0a-4548-b692-119ccbff1c5a
# Allow MLDatasets to download dataset automatically
ENV["DATADEPS_ALWAYS_ACCEPT"]=true;

# ╔═╡ efad7589-8ae0-40a5-9122-78ecf5c14c83
function load_dataset(::Type{D}) where D <: Union{MNIST, FashionMNIST}
    imgs, labels = D(:train)[:]
    x_train = reshape(imgs, 28, 28, 1, :)
	y_train = onehotbatch(labels, 0:9)

    imgs, labels = D(:test)[:]
    x_test = reshape(imgs, 28, 28, 1, :)
	y_test = onehotbatch(labels, 0:9)

	train_data = ComponentArray(;X=x_train, y=y_train)
	test_data = ComponentArray(;X=x_test, y=y_test)
	return (x_train, y_train, x_test, y_test)
end

# ╔═╡ d1d66c14-689c-44f1-a6c9-c539def969fe
x_train, y_train, x_test, y_test = load_dataset(MNIST);

# ╔═╡ 7dfccd5e-a25c-4629-b6f2-c20a74aec161
md"""
Since Julia arrays are stored in column-major order, we need to prepare an array of images in the form of `(W, H, C, N)`, where:
- W: image width
- H: image height
- C: number of channels (e.g., RGB or grayscale)
- N: number of images, mini batch size
"""

# ╔═╡ d35315ff-5722-4968-b46e-eb6859ecdaf6
begin
    const W = 28
    const H = 28
    const C = 1  # each image in MNIST is a gray scale, therefore C is 1
    const MINIST_DATA_SIZE = 60000
	@assert size(x_train) == (W, H, C, MINIST_DATA_SIZE)
end

# ╔═╡ 578b517e-f688-4af7-8fb3-3f52ea8a61dc
begin
	const NUM_CLASSES = 10
	@assert size(y_train) == (NUM_CLASSES, MINIST_DATA_SIZE)
end

# ╔═╡ 8594f276-42a0-4aaa-bdd2-362978e5629f
md"""
`y_train` contains $(MINIST_DATA_SIZE) elements that represent class id out of $(NUM_CLASSES).
"""

# ╔═╡ 7e3706f9-1947-49e9-8a9b-56fdfa674fca
md"""
## Visualizing the training dataset

Let's see what images we use for training. PlutoUI provides an
easy way to create an interactive UI component selecting data by
index.
"""

# ╔═╡ e1d584d7-7b93-4201-8315-aa01c5f58c67
@bind train_example_idx Select(1:100)

# ╔═╡ 837f620f-c4d4-4141-848a-ed7acd1d99ef
begin
	# Keep in mind we need to apply `transpose` function to visualize images properly.
	x_train_example = transpose(dropdims(x_train[:, :, :, train_example_idx], dims=3))
	y_train_example = onecold(y_train[:, train_example_idx], 0:9)
end

# ╔═╡ b76032b6-e1cf-44b6-af3b-2bc8f403f31d
md"""
In the following, we'll be selecting `train_example_idx`=$(train_example_idx). The number $(y_train_example) is written on the image.
"""

# ╔═╡ b186ba8c-5851-4b92-a752-5dc4d7632f26
md"""
`train_example_idx` = $(train_example_idx)

`y_train_example` = $(y_train_example)

$(colorview(Gray, x_train_example))
"""

# ╔═╡ 9b31963f-5c19-4c5a-b24d-08bdc65cb6d4
md"""
## Setting up the DataLoader

[MLUtils.jl](https://github.com/JuliaML/MLUtils.jl) package provides DataLoader that is similar to [DataLoader in PyTorch](https://pytorch.org/tutorials/beginner/basics/data_tutorial.html#preparing-your-data-for-training-with-dataloaders).
"""

# ╔═╡ b76a8e90-025b-4397-b98f-79f8bd330dbb
begin
	batchsize = 64
...
# ╔═╡ bb47806a-d48d-445e-abc2-601db2a06f38
md"""
The `first` function is used to retrieve a mini-batch of data from a data loader, specifically the `train_loader`. We see that `x_train_batch` has a batch of `batchsize`=$(batchsize) images.
"""

# ╔═╡ 3589f7f8-979f-4f1f-bc89-955bbd7234ca
let
	x_train_batch, y_train_batch = first(train_loader)
	@assert size(x_train_batch) == (W, H, C, batchsize)
	@assert size(y_train_batch) == (NUM_CLASSES, batchsize)
end

# ╔═╡ 9f2bcc96-0a34-4218-ac06-349497c403d2
md"""
## Creating a model

We utilize affine transformation layers `Dense` with ReLU (Rectified Linear Unit) activation functions `relu` to construct an image recognition model. The architecture consists of the following layers, which are written as follows:
"""

# ╔═╡ 4110d46d-1844-46ec-9c1f-835ba1839d5c
...
# ╔═╡ 81303b36-cefe-4081-994e-1ca3b2a53a0a
md"""
The architecture consists of the following layers:
* The `Chain` corresponds to [Sequential](https://pytorch.org/docs/stable/generated/torch.nn.Sequential.html#sequential) in PyTorch.
* The `Dense` layer corresponds to [Linear](https://pytorch.org/docs/stable/generated/torch.nn.Linear.html#linear) in PyTorch.
* The `FlattenLayer` simply flattens the image data (matrix) into a vector.
"""

# ╔═╡ 92b5de24-e447-455a-a1de-4eb936eac0c7
md"""
### Initializing parameters and states

As described in Lux.jl documentation in [Design Principles](https://lux.csail.mit.edu/dev/introduction/overview#Design-Principles) `Layers are pure functions` to make it both compiler and autodiff friendly. All layers are also deterministic given the parameters and state. Therefore user should pass parameter object `ps` and state object `st` to get output from a model.
"""

# ╔═╡ 2653d121-acee-4df4-8eb3-4e3c509f8c34
begin
	model = create_model()
...
# ╔═╡ c4413a7c-c71c-46bc-82e6-27bb07d88895
md"""
To obtain the output `y` corresponding to the input `x`, users in Lux.jl should call:
...
# ╔═╡ ca1299cc-8d9a-4ab7-8dc8-885370a04e4a
let
	x_train_batch, y_train_batch = first(train_loader)
	ŷ_apply, st_apply = Lux.apply(model, x_train_batch, ps, st)
	size(ŷ_apply) == (10, batchsize)

	ŷ_call, st_call = model(x_train_batch, ps, st)
	size(ŷ_call) == (10, batchsize)
end

# ╔═╡ 08d42c42-f549-4975-8a85-3d4520e3a51d
md"""
The second output `st_apply` or `st_call` is a state object. This becomes important when defining layers like BatchNorm, WeightNorm, etc. However, since we don't use these complicated layers in this notebook, users can ignore it for now.
"""

# ╔═╡ 6850c56a-1b06-43f3-ba23-6019d1350ce6
md"""
## Starting training

It's time to train our model! As we iterate through the training steps, we expect the model to learn the underlying structure of the data, allowing it to recognize numbers in given images. To evaluate its accuracy, we define a function to measure the accuracy of the model.
"""

# ╔═╡ 80426e4e-a204-4a9f-9ee9-03043c7f8725
function accuracy(model, ps, st, dataloader)
	...
	for (x_cpu, y_cpu) in dataloader
		x = x_cpu |> device
		y = y_cpu |> device
		target_class = onecold(y_cpu)
		ŷ = model(x, ps, st)[1] |> cpu_dev
		predicted_class = onecold(ŷ)
...
# ╔═╡ a2ee6c97-9e94-4382-a4dc-4b6d4b39955a
md"""
Here is a training function
"""

# ╔═╡ ba2dd32b-183d-4058-9df6-517e202fdf56
function train(::Type{D}, nepochs, batchsize) where D <: Union{MNIST, FashionMNIST}
	x_train, y_train, x_test, y_test = load_dataset(D)
	train_loader = DataLoader((x_train, y_train); batchsize, shuffle=true)
	test_loader = DataLoader((x_test, y_test); batchsize, shuffle=false)
	model = create_model()
	...
	for epoch in 1:nepochs
		...
		train_acc = accuracy(model, ps, st, train_loader)
		test_acc = accuracy(model, ps, st, test_loader)
		@info epoch train_acc test_acc
	end
	tstate.model, tstate.parameters, Lux.testmode(tstate.states)
end

# ╔═╡ 9378a120-bc3b-4faa-982a-3c8a348078c9
# Press "shift + enter" to run this sell
@time trained_model, trained_ps, trained_st  = let
	nepochs = 10
	batchsize = 64
	train(MNIST, nepochs, batchsize)
end

# ╔═╡ 92858a87-5f66-4aa5-8440-24700e44d011
md"""
## Predicting Digits in Images

Now, let's see if our trained model can correctly identify a digit in an image that the user provides.
"""

# ╔═╡ 2b8e118b-3450-4b19-80c9-a76f9b685d54
md"""
### Visualizing the test dataset

Let's see what images we use for test.
"""

# ╔═╡ 2c8e9b72-a489-4583-a3e7-8125525f70eb
@bind test_example_idx Select(1:100)

# ╔═╡ 2cb16fa6-a053-4402-ac91-13d90d88f671
md"""
---

Below is a snippet to construct a dashboard.
"""

# ╔═╡ eee5656b-b1b1-4de1-9eaa-e5af4ad397b8
begin
	ŷ_test_, _ = trained_model( 
		x_test[:, :, :, test_example_idx], 
		trained_ps, 
		trained_st
	)
	prob = softmax(ŷ_test_)
	ŷ_test = only(onecold(ŷ_test_, 0:9))
	p = prob[argmax(prob)]
	pstr = @sprintf "%.1f" 100p
end

# ╔═╡ fe848862-e39e-41e3-b05c-836096c85cf5
begin
	x_test_example = transpose(
			dropdims(x_test[:, :, :, test_example_idx], dims=3)
		)
	y_test_example = onecold(y_test[:, test_example_idx], 0:9)
end

# ╔═╡ dc71a1c3-147f-4fa2-9169-6673225ac67d
md"""
In the following, we'll be selecting `test_example_idx`=$(test_example_idx). The number $(y_test_example) is written on the image.
"""

# ╔═╡ 9c20f083-2313-46fd-acad-4cbf19a46340
md"""
`test_example_idx` = $(test_example_idx)

`y_test_example` = $(y_test_example)

Based on our trained model, we predict `ŷ_test` = $(ŷ_test) will be approximately $(y_test_example) with a confidence level of $(pstr)%"

$(colorview(Gray, x_test_example))
"""
...
manifest_format = "2.0"
project_hash = "80114bba95474ada45f8bdd70865a061d8237f99"
...
[[deps.Lux]]
deps = ["ADTypes", "Adapt", "ArgCheck", "ArrayInterface", "ChainRulesCore", "Compat", "ConcreteStructs", "ConstructionBase", "EnzymeCore", "FastClosures", "Functors", "GPUArraysCore", "LinearAlgebra", "LossFunctions", "LuxCore", "LuxDeviceUtils", "LuxLib", "MacroTools", "Markdown", "Preferences", "Random", "Reexport", "Setfield", "Statistics", "UnrolledUtilities", "WeightInitializers"]
git-tree-sha1 = "7fed517b397ded64f6996d71abdb4f5a50928a66"
...
[[deps.LuxCore]]
deps = ["DispatchDoctor", "Functors", "Random", "Setfield"]
git-tree-sha1 = "81e299585b8976b6a9996512e6670b94ec931e23"
uuid = "bb33d45b-7691-41d6-9220-0943567d0623"
version = "0.1.18"

[[deps.LuxDeviceUtils]]
deps = ["Adapt", "ChainRulesCore", "Functors", "LuxCore", "Preferences", "Random"]
git-tree-sha1 = "4cb65a307dd7ad9bce28a2f8b4e31c5fb446e762"
...
[[deps.LuxLib]]
deps = ["ArrayInterface", "ChainRulesCore", "DispatchDoctor", "EnzymeCore", "FastBroadcast", "FastClosures", "LinearAlgebra", "LuxCore", "LuxDeviceUtils", "Markdown", "NNlib", "Random", "Reexport", "Statistics"]
git-tree-sha1 = "e65f55d2480015c2c6ca0ef862471f3300ff3bc7"
...
git-tree-sha1 = "1288e6db94d98f7d194454452176b82edb25b32c"
...
deps = ["ChainRulesCore", "GPUArraysCore", "LinearAlgebra", "PartialFunctions", "Random", "SpecialFunctions", "Statistics"]
